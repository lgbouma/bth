...
#PRIMARIES
#boxed eqn 17/10/13.2. Define a lambda function for p1_ra so that you
#can use quad to integrate over the inf.

p1_ra = lambda r_a: \
        (1 + (( (r_p/r_a)**2 - 1)**(1/α))**α)**(-3) * \
        (( (r_p/r_a)**2 - 1)**(1/α))**(γ+β) * \
        (1+(( (r_p/r_a)**2 - 1)**(1/α))**α)**(3/2) * \
        (B*(1+(( (r_p/r_a)**2 - 1)**(1/α))**α)**(3/2))*( 1 - (B*(1+(( (r_p/r_a)**2 - 1)**(1/α))**α)**(3/2)))**(-1)
...
ax.set_ylabel(
'probability that point on sky has a detected planet with radius $r_a$',
fontsize='xx-small')
...
f.savefig('prob_apparent_detected_rate_density.pdf', dpi=300, bbox_inches='tight')
