...
For a population with fixed  $\gamma_R$, since ${\rm S/N} \propto \mathcal{D} 
L^{1/2} d^{-1}$,
\begin{equation}
...
Now let the planets have a power-law radius distribution, ${\rm d}N_p/{\rm 
...
\begin{equation}
\frac{N_{\rm planets}(R_p^{\rm min}<R_p <R_p^{\rm max})}{N_{\rm stars}} = 
\Gamma_0 \int_{R_p^{\rm min}}^{R_p^{\rm max}} R_p^\delta \,{\rm d}R_p,
\end{equation}
