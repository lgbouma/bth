...
\int_{0.1^{7/2} + 1}^{2} u^{3/2} (u-1)^{5/7} \,{\rm d}u ,
...
of the Gauss hypergeometric function, $_2F_1$. Once evaluated, and after 
normalizing ${\rm prob}(N_d)$, to 4 significant digits it gives
\begin{equation}
\langle N_d \rangle = 1.601 \left(\frac{4\pi n_d}{3}\right) d_{\rm max,s}^3.
\end{equation}
This gives the useful expression
\begin{align}
\frac{\langle N_d \rangle}{N_s} &= 1.601\, \frac{n_d}{n_s} \\
&= 1.601 \left(\frac{{\rm BF}}{1-{\rm BF}}\right).
\end{align}
Taking the sun-like star binary fraction of ${\rm BF} = 0.44$, this gives
$\langle N_d \rangle / N_s = 1.258$.
...
single systems, there are now $\approx 1.3$ times as many.
